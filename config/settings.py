...
class Settings(BaseSettings):
    """Application settings with secure Railway-friendly defaults."""
    ...
    # - zoom 17 is much lighter than zoom 20/21
    # - tile size 256 is MapTiler-standard and avoids huge stitched images
    # - max tiles capped to reduce timeout/memory risk
    max_tiles_per_request: int = Field(default=100, ge=1, le=500)
    tile_size: int = Field(default=256, ge=256, le=2048)
    zoom_level: int = Field(default=17, ge=15, le=21)
    request_timeout: int = Field(default=60, ge=5, le=120)
    max_concurrent_requests: int = Field(default=5, ge=1, le=50)

    # Processing settings
    batch_size: int = Field(default=1, ge=1, le=16)
    confidence_threshold: float = Field(default=0.25, ge=0.1, le=0.99)
    iou_threshold: float = Field(default=0.45, ge=0.1, le=0.9)
    max_image_dimension: int = Field(default=4096, ge=1024, le=16384)

    # Memory management
    max_memory_mb: int = Field(default=2048, ge=512, le=32768)
    cleanup_interval_sec: int = Field(default=30, ge=10, le=600)

    # Storage paths
    # Use /tmp on Railway because app filesystem can be ephemeral / restricted.
    ...
    # Model paths
    roof_model_path: str = "/tmp/models/roof_detector.pt"
    damage_model_path: str = "/tmp/models/damage_detector.pt"

    # Rate limiting
    rate_limit_requests_per_minute: int = Field(default=10, ge=1, le=1000)
...
def get_settings() -> Settings:
    ...
    Settings are loaded once for performance.
    """
    return Settings()
...
def validate_settings(require_api_key: bool = False) -> dict:
    ...
    Args:
        require_api_key: If True, fail if MapTiler API key is missing.
    ...
    try:
        ...
        if settings.max_memory_mb < 1024:
            result["warnings"].append(
                f"Low memory limit ({settings.max_memory_mb}MB) may cause issues"
            )
